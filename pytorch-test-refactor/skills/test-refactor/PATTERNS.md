...
## Pattern 8: DEVICE_GENERIC Class Already Using `instantiate_parametrized_tests`

When a class is classified as DEVICE_GENERIC but already uses `instantiate_parametrized_tests`, do NOT also add `instantiate_device_type_tests`. They are mutually exclusive — both parametrize test methods, and combining them causes `TypeError` from double-parametrization.
